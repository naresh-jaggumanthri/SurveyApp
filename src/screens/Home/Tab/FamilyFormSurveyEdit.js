...
const respondantData=[
 ...
 const [familyMemberCount, setFamilyMemberCount] = useState(0);
  ...
  const [isLpgConnectionUnderUjjwala, setIsLpgConnectionUnderUjjwala] = useState(null);
  const [havingLabourCards, setHavingLabourCards] = useState(null);
  const [isCoveredUnderNSKY, setIsCoveredUnderNSKY] = useState(null);
  const [isFamilyInvolvedInWeavingOrHandloom, setIsFamilyInvolvedInWeavingOrHandloom] = useState(null);
  const [isFamilyCoveredUnderPOHI_LoomsScheme, setIsFamilyCoveredUnderPOHI_LoomsScheme] = useState(null);
  ...
  const [isIrrigationFacilityAvailable, setIsIrrigationFacilityAvailable] = useState(null);
  ...
  const [isEnrolledUnderShramYogiMaandhan, setIsEnrolledUnderShramYogiMaandhan] = useState(null);
  const [isCoveredUnderAyushmanBharat, setIsCoveredUnderAyushmanBharat] = useState(null);
  const [hasElectricityConnection, setHasElectricityConnection] = useState(null);
  const [hasIndividualHouseholdLatrine, setHasIndividualHouseholdLatrine] = useState(null);
  const [hasMGNREGSJobCard, setHasMGNREGSJobCard] = useState(null);
  const [hasRuralHousingSchemeHouse, setHasRuralHousingSchemeHouse] = useState(null);
  const [hasFamilyMemberMigratedLast3Years, setHasFamilyMemberMigratedLast3Years] = useState(null);
  const [takenAdvanceForMigrationFromMiddleman, setTakenAdvanceForMigrationFromMiddleman] = useState(null);
  const [minorChildrenAccompaniedMigration, setMinorChildrenAccompaniedMigration] = useState(null);
  ...
  if (editData && formikRef.current) {
    ...
    // 🔥 ALSO sync UI-only state (important)
    setHeadOfTheHouseholdGender(editData?.householdBasicProfile?.headOfTheHouseholdGender);
    setIsWomenInSHG(editData?.householdBasicProfile?.isWomenCoveredUnderSHG);
    setHasRationCard(editData?.householdBasicProfile?.hasRationCard);
    setFamilyMemberCount(editData?.householdBasicProfile?.totalFamilyMembers || 0);
  }
}, [editData]);
